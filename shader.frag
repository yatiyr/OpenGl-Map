...
void main() {
  ...
  vec3 half_vec = (ToLightVector+ToCameraVector)/length(ToCameraVector+ToLightVector);

  vec4 ambient = vec4(ka*Ia);

  float diff_cos = max(dot(ToLightVector,vertexNormal),0.0);

  float diff_cos_h = dot(vertexNormal,half_vec);

  //vec4 diffuse = vec4( ((diff_cos*kd*Id)/(distance_v)).x,((diff_cos*kd*Id)/(distance_v)).y,((diff_cos*kd*Id)/(distance_v)).z,1.0);

  vec4 diffuse = vec4( diff_cos*kd*Id);

  vec4 specular = vec4( pow(max(diff_cos_h,0.0),specExp)*ks*Is);

  // compute the color using the following equation

  color = vec4(clamp( textureColor.xyz * vec3(ambient + specular + diffuse ), 0.0, 1.0), 1.0); 
